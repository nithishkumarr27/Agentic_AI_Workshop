...
function App() {
  ...
  // Quiz functions
  const startQuiz = (intervention, isPreTest = true) => {
    if (!intervention?.assessment) {
      setError('This intervention has no assessment data');
      return;
    }
    
    const testType = isPreTest ? 'pre_test' : 'post_test';
    if (!intervention.assessment[testType]?.length) {
      setError(`No ${testType} questions available`);
      return;
    }

    setQuiz({
      questions: intervention.assessment[testType],
      answers: Array(intervention.assessment[testType].length).fill(null),
      results: null,
      isPreTest,
  ...
  const validateQuiz = () => {
    const { questions, answers, currentIntervention } = quiz;
    let correct = 0;
  ...
  const proceedAfterQuiz = async () => {
    const { results, isPreTest, currentIntervention } = quiz;
    
    if (!isPreTest && results.passed) {
      await storeQuizResults(results.score);
      await runRoadmapAdjustment();
    }
    
    setQuiz({ questions: [], answers: [], results: null, isPreTest: true, currentIntervention: null });
  };

  const storeQuizResults = async (score) => {
    try {
      await callAgent('store-results', {
        learnerId,
        interventionId: quiz.currentIntervention.concept,
        score,
        isPreTest: quiz.isPreTest
      });
    } catch (err) {
      setError('Failed to store quiz results');
    }
  };
              ...
              <div className="space-y-3">
                {Array.isArray(detectionResults.candidates) ? (
                  ...
                  <div className="p-3 bg-white rounded-lg border border-red-100">
                    <p className="mt-1 text-sm text-gray-600">{detectionResults.candidates}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="mt-4 p-4 bg-teal-50 rounded-lg border border-teal-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 bg-white rounded-lg border border-teal-100">
                  ...
                  <button 
                    onClick={() => startQuiz(interventionResults.interventions[0], true)} 
                    className="mt-3 w-full py-2 bg-teal-600 text-white rounded hover:bg-teal-700"
                  >
                    Start Assessment
                  </button>
                </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              <h2 className="text-xl font-bold mb-4">
                {quiz.isPreTest ? 'Pre-Test' : 'Post-Test'}: {quiz.currentIntervention.concept}
              </h2>
              ...
              {!quiz.results ? (
                <>
                  ...
                  <div className="flex justify-between mt-6">
                    <button
                      onClick={() => setQuiz({ questions: [], answers: [], results: null, isPreTest: true, currentIntervention: null })}
                ...
                <div>
                  <div className={`p-4 rounded-lg mb-6 text-center ${
                    ...
                    <p>Passing Threshold: {quiz.currentIntervention.assessment.success_threshold}%</p>
                  </div>
                  ...
                  >
                    {quiz.isPreTest ? 'Start Learning' : 'Continue to Roadmap'}
                  </button>
                </div>
              )}
            </div>
