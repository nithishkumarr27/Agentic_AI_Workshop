...
from rag_corpus_data import RAG_CORPUS
...
def generate_intervention_tool(misconception: dict) -> dict:
    """Generates learning interventions with interactive content for a classified misconception."""
    # Ensure required fields exist with defaults
    ...
    - MicroChallenges: Array of 3 short coding challenges with solutions
    - Explanation: Technical explanation with examples
    - InteractiveContent: Array of 2 interactive content objects with:
        type: [code_simulator|quiz|video|interactive_diagram]
        title: Short title
        description: 1-2 sentences
        content: Detailed content
    - Assessment: 
        pre_test: Array of 3 multiple-choice questions
        post_test: Array of 3 multiple-choice questions (different than pre_test)
        success_threshold: Minimum percentage score
    - EstimatedTime: Minutes required for completion
    - Metrics: Object tracking time_spent and success_rate
    
    Output ONLY as JSON with keys: 
        analogy, micro_challenges, explanation, interactive_content, 
        assessment, estimated_time, metrics
    """
    ...
    try:
        ...
        result.setdefault("interactive_content", [])
...
def validate_quiz():
    data = request.json
    learner_id = data.get('learnerId')
    intervention_concept = data.get('interventionId')
    answers = data.get('answers', [])
    pre_test = data.get('isPreTest', False)
    
    try:
        # Find the most recent intervention for this learner and concept
        intervention = misconceptions_col.find_one({
            "learner_id": learner_id,
            "type": "classified",
            "misconception.concept": intervention_concept
        }, sort=[("timestamp", -1)])
        
        if not intervention:
            return jsonify({
                "error": f"Intervention not found for learner {learner_id} and concept {intervention_concept}",
                "available_interventions": list(misconceptions_col.distinct(
        ...
        # Get correct answers
        test_type = "pre_test" if pre_test else "post_test"
        questions = intervention.get("misconception", {}).get("assessment", {}).get(test_type, [])
        
        if not questions:
            return jsonify({
                "error": f"No {test_type} questions found for this intervention",
        ...
        # Update metrics
        update_data = {
            f"metrics.{'pre_test_score' if pre_test else 'post_test_score'}": score,
            "metrics.last_updated": datetime.utcnow()
        }
        
        if not pre_test:
            update_data["metrics.completion_count"] = intervention.get("metrics", {}).get("completion_count", 0) + 1
            update_data["metrics.last_attempt"] = datetime.utcnow()
        ...
        return jsonify({
            "score": score,
            "passed": score >= intervention.get("misconception", {}).get("assessment", {}).get("success_threshold", 70),
            "results": results,
            "metrics": intervention.get("metrics", {})
        })
        
    except Exception as e:
        print(f"Quiz validation error: {str(e)}")
...
def store_results():
    ...
    is_pre_test = data.get('isPreTest', False)
    
    try:
        # Update metrics
        update_field = f"metrics.{'pre_test_score' if is_pre_test else 'post_test_score'}"
        
        result = misconceptions_col.update_one(
            {
                "learner_id": learner_id,
                "misconception.concept": intervention_concept
            },
            {
                ...
                "$inc": {
                    "metrics.completion_count": 0 if is_pre_test else 1
                }
            },
            upsert=False
        )
        ...
        if result.modified_count == 0:
            return jsonify({
                "warning": "No document was updated",
        ...
        print(f"Results storage error: {str(e)}")
        return jsonify({"error": str(e)}), 500
